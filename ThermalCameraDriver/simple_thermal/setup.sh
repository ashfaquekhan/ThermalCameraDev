...
# Simple compilation script for thermal camera test
echo "🔧 Compiling thermal camera test..."

# Save the fixed test program
cat > thermal_test.cpp << 'EOF'
#include <iostream>
#include <cstring>
#include <unistd.h>
#include <signal.h>
...
g++ -std=c++11 -o thermal_test thermal_test.cpp \
    -I../../include \
    -L../../libs \
    -Wl,-rpath,../../libs \
    $(pkg-config --cflags opencv4 2>/dev/null || echo "-I/usr/include/opencv4") \
    $(pkg-config --libs opencv4 2>/dev/null || echo "-lopencv_core -lopencv_imgproc -lopencv_highgui") \
    -liruvc -lirparse -lirprocess -lirtemp -lusb-1.0 -lpthread

if [ $? -eq 0 ]; then
    echo "✅ Compilation successful!"
    echo ""
    echo "📷 To run the test:"
    echo "   sudo ./thermal_test"
    echo ""
    echo "💡 The test will show you different temperature conversion methods"
    echo "   and help identify which one gives correct room temperature."
else
    echo "❌ Compilation failed!"
    echo ""
    echo "🔍 Troubleshooting:"
    echo "1. Check if headers exist:"
    echo "   ls ../../include/*.h"
    echo ""
    echo "2. Check if libraries exist:"
    echo "   ls ../../libs/*.so"
    echo ""
    echo "3. Try direct compilation:"
    echo "   g++ -std=c++11 thermal_test.cpp -I../../include -L../../libs -liruvc -lirtemp -lusb-1.0 -lopencv_core -lopencv_highgui -lopencv_imgproc"
fi