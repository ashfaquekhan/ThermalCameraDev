...
static const int MAIN_X=0, MAIN_Y=0, MAIN_W=496, MAIN_H=356;   // depth-on-thermal overlay
static const int COLX=500, COLW=300;                 // right feed column
static const int F_H=170;                            // each feed height
static const int F1Y=4, F2Y=182;                     // arducam-cam / depth (no thermal feed)
...
int main(){
    ...
    while(g_running&&is_streaming){
        ...
        // arducam camera feed = amplitude/confidence (auto-gain grayscale), separate
        cv::Mat amp;
        if(!conf_f.empty()){ cv::normalize(conf_f,amp,0,255,cv::NORM_MINMAX,CV_8U); cv::cvtColor(amp,amp,cv::COLOR_GRAY2BGR); }
        feed(amp,F1Y,"ARDUCAM CAM",cv::Scalar(200,200,0));
        // depth feed, separate
        cv::Mat rd;
        ...
        feed(rd,F2Y,"DEPTH",cv::Scalar(0,200,255));
        ...
        if(g_cmd_capture){ g_cmd_capture=false;
            ...
            if(!rd.empty())cv::imwrite(b+"_depth_raw.png",rd); if(!amp.empty())cv::imwrite(b+"_arducam_cam.png",amp);
